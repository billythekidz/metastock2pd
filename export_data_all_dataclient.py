...
pathExport = f'C:/AmiExportData/SYMBOL/ALL_TICK_{currentDateString}/'
...
get_all_symbol = pd.read_sql_query(f'SELECT SYMBOL FROM VN_Symbol', conn)
# df = df.set_index('DATE')
for index, record in get_all_symbol.iterrows(): 
    symbol = record['SYMBOL']  
    df = pd.read_sql_query(f'SELECT SYMBOL,DATE,[OPEN],VOLUME FROM VN_Intraday WHERE SYMBOL=\'{symbol}\'  ORDER BY DATE ASC', conn)
    with open(pathExport + f'{symbol}_TICK_{currentDateString}.csv'.replace('/','').replace('^',''), 'w', newline='') as mcFile:
